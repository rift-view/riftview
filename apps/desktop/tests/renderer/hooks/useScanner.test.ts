...
describe('useScanner', () => {
  it('calls selectProfile with first profile on mount', async () => {
    ...
    await waitFor(() =>
      expect(window.riftview.selectProfile).toHaveBeenCalledWith({ name: 'default' })
    )
  })
  ...
  it('aborts scan and shows toast when credentials are invalid', async () => {
    window.riftview.validateCredentials = vi.fn().mockResolvedValue({
      ok: false,
      error: 'ExpiredTokenException: The security token included in the request is expired'
    })
    window.riftview.startScan = vi.fn().mockResolvedValue(undefined)
    ...
    expect(window.riftview.startScan).not.toHaveBeenCalled()
