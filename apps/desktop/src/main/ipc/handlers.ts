...
export function registerHandlers(win: BrowserWindow): void {
  ...
  // CloudFront CRUD handlers — delegated to @riftview/cloud-scan wrappers
  ipcMain.handle(IPC.CF_CREATE, async (_e, params: CloudFrontParams) => {
    if (!clients) return { code: 1, error: 'No CloudFront client' }
    try {
      await cfCreateDistribution(clients, params)
      return { code: 0 }
    } catch (err) {
  ...
  ipcMain.handle(IPC.CF_UPDATE, async (_e, id: string, params: CloudFrontEditParams) => {
    if (!clients) return { code: 1, error: 'No CloudFront client' }
    try {
      await cfUpdateDistribution(clients, id, params)
      return { code: 0 }
    } catch (err) {
  ...
  ipcMain.handle(IPC.CF_DELETE, async (_e, id: string) => {
    if (!clients) return { code: 1, error: 'No CloudFront client' }
    try {
      await cfDeleteDistribution(clients, id)
      return { code: 0 }
    } catch (err) {
  ...
  ipcMain.handle(IPC.CF_INVALIDATE, async (_e, id: string, cfPath: string) => {
    if (!clients) return { code: 1, error: 'No CloudFront client' }
    try {
      await cfCreateInvalidation(clients, id, cfPath)
      return { code: 0 }
    } catch (err) {
