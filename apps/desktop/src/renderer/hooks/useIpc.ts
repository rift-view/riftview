...
export function useIpc(): void {
  ...
  useEffect(() => {
    const unsubDelta = window.riftview.onScanDelta((delta) => {
    ...
    const unsubErrorDetail = window.riftview.onScanErrorDetail((detail) => {
      setError(detail.message)
    })
    const unsubKeypairs = window.riftview.onScanKeypairs((pairs: string[]) => {
      setKeyPairs(pairs)
    })
    return () => {
      unsubDelta()
