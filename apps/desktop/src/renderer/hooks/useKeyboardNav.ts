    ...
    function onKeyDown(e: KeyboardEvent): void {
      ...
      switch (e.key) {
        case 'j': {
          ...
          window.dispatchEvent(new CustomEvent('riftview:fitnode', { detail: { nodeId: node.id } }))
          break
        }
        case 'k': {
          if (sortedNodes.length === 0) return
        ...
        case 'Enter': {
          ...
          window.dispatchEvent(
            new CustomEvent('riftview:fitnode', { detail: { nodeId: selectedId } })
          )
          break
        }
        case '/': {
          e.preventDefault()
          window.dispatchEvent(new CustomEvent('riftview:open-search'))
          break
        }
        case 'Escape': {
        ...
        case 'r': {
          e.preventDefault()
          window.riftview.startScan().catch(() => {})
          break
        }
        case '?': {
          e.preventDefault()
