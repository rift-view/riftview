...
import { evaluatePolicy } from '../../iam/evaluator'
import type { PolicyStatement } from '../../iam/evaluator'
...
describe('evaluatePolicy', () => {
  ...
  it('flags AdministratorAccess managed policy by name', () => {
    ...
    expect(
      findings.some((f) => f.severity === 'critical' && f.title.includes('AdministratorAccess'))
    ).toBe(true)
  })

  it('flags wildcard action + wildcard resource (Action:* Resource:*)', () => {
    const findings = evaluatePolicy({ Statement: [allow('*', '*')] })
    expect(findings.some((f) => f.severity === 'critical' && f.title.includes('Wildcard'))).toBe(
      true
    )
  })
  ...
  it('flags sts:AssumeRole with wildcard resource', () => {
    ...
    expect(findings.some((f) => f.severity === 'warning' && f.title.includes('AssumeRole'))).toBe(
      true
    )
  })
  ...
  it('flags Principal object with wildcard value { AWS: "*" }', () => {
    const findings = evaluatePolicy({
      Statement: [allow('s3:GetObject', 'arn:aws:s3:::bucket/*', { AWS: '*' })]
    })
    expect(findings.some((f) => f.severity === 'warning' && f.title.includes('Public'))).toBe(true)
  })
  ...
  it('flags cross-account trust in Principal', () => {
    ...
    expect(
      findings.some((f) => f.severity === 'info' && f.title.toLowerCase().includes('cross-account'))
    ).toBe(true)
  })
  ...
  it('returns empty for benign scoped policy', () => {
    const findings = evaluatePolicy({
      Statement: [allow('s3:GetObject', 'arn:aws:s3:::my-bucket/*')]
    })
    expect(findings).toHaveLength(0)
  })
