# RiftView M4 — Theme System Implementation Plan
...
```bash
cd /Users/julius/AI/proj1/.worktrees/m4/riftview/riftview
...
Open `src/preload/index.ts`. Inside `contextBridge.exposeInMainWorld('riftview', { ... })`, add:

```ts
getThemeOverrides: () => ipcRenderer.invoke(IPC.THEME_OVERRIDES),
```

Open `src/preload/index.d.ts`. Inside the `riftview` interface, add:
...
Update it to also call `getThemeOverrides` and inject the override `<style>` tag. The existing effect also calls `window.riftview.listProfiles().then(setProfiles)` — preserve that call:
