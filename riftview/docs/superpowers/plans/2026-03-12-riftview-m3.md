# RiftView M3 Implementation Plan
...
**Working directory:** `riftview/` (all paths below are relative to it)
...
Find where `window.riftview.runCli(paramsRef.current)` is called and replace with:
...
it('setPendingCommand stores command chain', () => {
  ...
  expect(store.getState().pendingCommand).toEqual([
...
const handleRun = async () => {
  if (pendingCommand) {
    ...
    setIsRunning(true)
    clearCliOutput()
    await window.riftview.runCli(pendingCommand)
...
Note: identify how `window.riftview` is mocked (likely `vi.stubGlobal` or `beforeEach` setup). Use the same mock object name.

- [ ] **Step 2: Write failing test**

In `src/renderer/components/modals/__tests__/CreateModal.test.tsx`, add a new `it` block. Adapt the mock references to match what already exists in the file (the mock object may be called `mockRiftView`, `mockBridge`, or set up inline):
  ...
  return (
    <div style={overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={panel}>
        ...
        <div style={{ fontSize: 10, color: '#555', marginBottom: 16 }}>Theme — coming in M4</div>
  ...
  return (
    <div style={overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={dialog}>
        ...
        <div style={{ color: '#555', fontSize: 9, marginBottom: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button
            disabled={!confirmed}
            onClick={() =>
              onConfirm({ skipFinalSnapshot: skipSnapshot || undefined, force: force || undefined })
            }
            style={{
  ...
  return (
    <div>
      ...
      <label style={row}>
        ...
        />
        <span style={{ fontSize: 10, color: '#aaa' }}>Multi-AZ</span>
      </label>
      <label style={row}>
        <input
          type="checkbox"
        ...
        />
        <span style={{ fontSize: 10, color: '#aaa' }}>Deletion protection</span>
      </label>
    </div>
  )
}
  ...
  return (
    <div>
      <label style={row}>
        ...
        />
        <span style={{ fontSize: 10, color: '#aaa' }}>Versioning</span>
      </label>
      <label style={row}>
        <input
          type="checkbox"
        ...
        />
        <span style={{ fontSize: 10, color: '#aaa' }}>Block public access</span>
      </label>
    </div>
  )
}
...
export default function SgEditForm({ node, onChange }: Props) {
  ...
  return (
    <div>
      <div style={{ fontSize: 9, color: '#555', textTransform: 'uppercase', marginBottom: 6 }}>
        Inbound rules
      </div>
      {rules.map((r, i) => (
...
```tsx
{
  ;(selectedNode.type === 'ec2' || selectedNode.type === 'rds') && (
    <div style={{ marginTop: 8 }}>
      <div style={{ fontSize: 8, color: '#555', textTransform: 'uppercase', marginBottom: 4 }}>
        Quick actions
      </div>
      <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
        {selectedNode.status !== 'stopped' && (
          <button
            onClick={() => onQuickAction(selectedNode, 'stop')}
            style={{
              background: '#1a2332',
        ...
        {selectedNode.status === 'stopped' && (
          <button
            onClick={() => onQuickAction(selectedNode, 'start')}
            style={{
              background: '#1a2332',
        ...
        <button
          onClick={() => onQuickAction(selectedNode, 'reboot')}
          style={{
            background: '#1a2332',
            border: '1px solid #64b5f6',
            borderRadius: 2,
  ...
  return (
    <div>
      ...
      <label style={row}>
        ...
        />
        <span style={{ fontSize: 10, color: '#aaa' }}>Publicly accessible</span>
      </label>
    </div>
  )
}
...
export default function LambdaForm({ onChange, showErrors }: Props) {
  ...
  const filteredSubnets = form.vpcId
    ? subnets.filter((s) => s.metadata.vpcId === form.vpcId)
    : subnets
