...
describe('listRoutes', () => {
      ...
      Items: [
        {
          ...
          IntegrationUri: 'arn:aws:lambda:us-east-1:123:function:myFn'
        }
      ]
    })
  ...
  it('skips $default route', async () => {
    mockSend.mockResolvedValueOnce({
      Items: [{ RouteId: 'r2', RouteKey: '$default', Target: undefined }]
    })
    mockSend.mockResolvedValueOnce({ Items: [] })
    const nodes = await listRoutes(mockClient, 'api1', 'us-east-1')
    expect(nodes).toHaveLength(0)
  ...
  it('sets lambdaArn undefined when integration is not AWS_PROXY', async () => {
    mockSend.mockResolvedValueOnce({
      Items: [{ RouteId: 'r3', RouteKey: 'POST /items', Target: 'integrations/i2' }]
    })
    mockSend.mockResolvedValueOnce({
      Items: [
        {
    ...
    listApis: vi.fn().mockResolvedValue([
      {
        ...
        status: 'running',
        region: 'us-east-1',
        metadata: { endpoint: '', protocolType: 'HTTP', corsOrigins: [] }
      }
    ]),
    listRoutes: vi.fn().mockResolvedValue([
      {
        ...
        status: 'running',
        region: 'us-east-1',
        metadata: { apiId: 'api1', routeId: 'r1', method: 'GET', path: '/test' },
        parentId: 'api1'
      }
    ])
  }))
...
export default function ApigwRouteNode({ data }: NodeProps<CloudNode>) {
  ...
  return (
    ...
    >
      ...
      {hasLambda && (
        <span title="Lambda integration" style={{ color: 'var(--cb-text-muted)', fontSize: 10 }}>
          λ
        </span>
      )}
    </div>
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span style={{ fontSize: 11, color: 'var(--cb-text)', fontWeight: 600 }}>{data.label}</span>
    ...
    <form onSubmit={handleSubmit}>
      ...
      {origins.map((o, i) => (
        <div key={i} style={{ display: 'flex', gap: 4 }}>
          ...
          />
          <button type="button" onClick={() => removeOrigin(i)} disabled={origins.length === 1}>
            −
          </button>
        </div>
      ))}
...
export default function ApigwRouteForm({ parentId, onSubmit, onCancel }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      />
      {path && !pathValid && (
        <span style={{ color: 'var(--cb-red)', fontSize: 11 }}>Path must start with /</span>
      )}
...
```tsx
<ContextMenuItem onClick={() => onCreateResource('apigw')}>New API Gateway</ContextMenuItem>
```
