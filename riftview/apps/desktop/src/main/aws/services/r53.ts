    ...
    return Promise.all(
      zones.map(async (item): Promise<CloudNode> => {
        const base: CloudNode = {
          ...
          region: 'global',
          metadata: {
            private: item.Config?.PrivateZone ?? false,
            recordCount: item.ResourceRecordSetCount ?? 0
          }
        }

        try {
          const rrsRes = await client.send(
            new ListResourceRecordSetsCommand({ HostedZoneId: item.Id })
          )
