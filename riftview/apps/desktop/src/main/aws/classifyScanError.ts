...
export function classifyScanError(err: unknown): ScanErrorDetail {
  ...
  if (
    msg.includes('invalidclienttoken') ||
  ...
  if (
    ...
    return {
      kind: 'permission',
      message:
        'IAM permission missing. See the onboarding panel for the required read-only actions.',
      raw
    }
  }
  ...
  if (
    ...
    msg.includes('region not enabled') ||
    msg.includes('region is disabled')
  ) {
    return {
