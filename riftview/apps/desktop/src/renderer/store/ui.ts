...
export const useUIStore = create<UIState>((set, get) => ({
  ...
  selectEdge: (info) =>
    set({ selectedEdgeId: info?.id ?? null, selectedEdgeInfo: info, selectedNodeId: null }),
  ...
  updateStickyNote: (id, content) =>
    set((s) => ({ stickyNotes: s.stickyNotes.map((n) => (n.id === id ? { ...n, content } : n)) })),
  ...
  addFilter: (filter) =>
    set((s) => ({
      activeFilters: [...s.activeFilters.filter((f) => f.id !== filter.id), filter]
    })),
  removeFilter: (id) =>
    set((s) => ({
      ...
      activeFilterTypes:
        id === 'sidebar-type' && s.activeFilterTypes.size > 0
          ? new Set<NodeType>()
          : s.activeFilterTypes
    })),
  clearFilters: () =>
    set({ activeFilters: [], activeSidebarType: null, activeFilterTypes: new Set<NodeType>() }),
  ...
  setZoneSize: (id, size) => set((s) => ({ zoneSizes: { ...s.zoneSizes, [id]: size } })),
  ...
  updateCustomEdgeLabel: (id, label) =>
    set((s) => ({
      customEdges: s.customEdges.map((e) => (e.id === id ? { ...e, label } : e))
    })),
  updateCustomEdgeColor: (id, color) =>
    set((s) => ({
  ...
  openTerminal: (nodeId, sessionId) =>
    set({ terminalNodeId: nodeId, terminalSessionId: sessionId }),
