...
export function TerminalPane(): React.JSX.Element | null {
  ...
  useEffect(() => {
    ...
    const cleanup = window.riftview?.onTerminalOutput?.((data) => {
      if (data.sessionId === sessionId) term.write(data.data)
    })

    term.onData((data) => {
      window.riftview?.sendTerminalInput?.(sessionId, data).catch(() => {})
    })

    return () => {
      cleanup?.()
      term.dispose()
      window.riftview?.closeTerminal?.(sessionId).catch(() => {})
    }
  }, [sessionId])
