...
export function buildCommands(params: CreateParams): string[][] {
  switch (params.resource) {
    ...
    case 'ec2': {
      ...
      if (params.securityGroupIds.length > 0)
        ec2Args.push('--security-group-ids', ...params.securityGroupIds)
      return [ec2Args]
    }
...
function buildLambdaCommands(p: LambdaParams): string[][] {
  ...
  if (p.vpcId && p.subnetIds && p.securityGroupIds) {
    args.push(
      '--vpc-config',
      `SubnetIds=${p.subnetIds.join(',')},SecurityGroupIds=${p.securityGroupIds.join(',')}`
    )
  }
  return [args]
}
...
function buildApigwRouteCommands(p: ApigwRouteParams): string[][] {
  return [
    ['apigatewayv2', 'create-route', '--api-id', p.apiId, '--route-key', `${p.method} ${p.path}`]
  ]
}
...
function buildDynamoCommands(p: DynamoParams): string[][] {
  ...
  return [
    [
      ...
      p.tableName,
      '--key-schema',
      `AttributeName=${p.hashKey},KeyType=HASH`,
      '--attribute-definitions',
      `AttributeName=${p.hashKey},AttributeType=S`,
      '--billing-mode',
      billingMode
    ]
  ]
...
function buildSecretCommands(p: SecretParams): string[][] {
  return [['secretsmanager', 'create-secret', '--name', p.name, '--secret-string', p.value]]
}
...
function buildR53ZoneCommands(p: R53ZoneParams): string[][] {
  return [
    [
      ...
      Date.now().toString(),
      '--hosted-zone-config',
      `Comment="",PrivateZone=${p.isPrivate}`
    ]
  ]
}
...
function buildSubnetCommands(p: CreateSubnetParams): string[][] {
  const args = ['ec2', 'create-subnet', '--vpc-id', p.vpcId, '--cidr-block', p.cidrBlock]
