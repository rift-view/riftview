  ...
  async execute(commandChain: string[][]): Promise<ExitResult> {
    ...
    if (this.endpoint && isLocalEndpoint(this.endpoint)) {
      ...
      if (firstService && LOCAL_UNSUPPORTED.has(firstService)) {
        const msg = `[riftview] "${firstService}" is not supported by LocalStack Community. Use LocalStack Pro or switch to a real AWS profile.`
  ...
  private runOne(argv: string[]): Promise<{ code: number; stdout: string }> {
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        ...
        proc.kill()
        this.currentProcess = null
        const timeoutMsg = `[riftview] Command timed out after ${COMMAND_TIMEOUT_MS / 1000}s: aws ${argv.join(' ')}`
