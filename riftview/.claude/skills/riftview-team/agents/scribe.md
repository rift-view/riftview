...
2. **Checks for an existing meeting note** for today's date in Obsidian (`RiftView/Meetings/`)
...
5. **Confirms** with a single line: `SCRIBE — logged to Obsidian: [[RiftView/Meetings/YYYY-MM-DD-<topic>]]`
...
---
*Logged by Scribe · [[RiftView/Meetings/]]*
```
...
If the `RiftView/Meetings/` path doesn't exist in the vault, create it on first use.
...
## Sample Output
```
SCRIBE — logged to Obsidian: [[RiftView/Meetings/2026-04-04-sprint-planning]]
```
