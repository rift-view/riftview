...
interface Props { onChange: (p: SfnParams) => void; showErrors?: boolean }
...
const BLANK_DEFINITION = JSON.stringify({ Comment: 'My state machine', StartAt: 'HelloWorld', States: { HelloWorld: { Type: 'Pass', End: true } } }, null, 2)
  ...
  return (
    <div>
      ...
      <input
        ...
        onChange={(e) => { setName(e.target.value); emit(e.target.value, type, roleArn, definition) }}
      />
      ...
      <select
        style={inp(false)}
        value={type}
        onChange={(e) => { setType(e.target.value as 'STANDARD' | 'EXPRESS'); emit(name, e.target.value as 'STANDARD' | 'EXPRESS', roleArn, definition) }}
      >
      ...
      <input
        ...
        onChange={(e) => { setRoleArn(e.target.value); emit(name, type, e.target.value, definition) }}
      />
      ...
      <textarea
        ...
        value={definition}
        onChange={(e) => { setDefinition(e.target.value); emit(name, type, roleArn, e.target.value) }}
      />
    </div>
  )
}
