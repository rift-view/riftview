...
interface Props { onChange: (p: DynamoParams) => void; showErrors?: boolean }
  ...
  return (
    <div>
      ...
      <input
        ...
        onChange={(e) => { setTableName(e.target.value); emit(e.target.value, hashKey, billingMode) }}
      />
      ...
      <input
        ...
        value={hashKey}
        placeholder="id"
        onChange={(e) => { setHashKey(e.target.value); emit(tableName, e.target.value, billingMode) }}
      />
      ...
      <select
        style={inp(false)}
        value={billingMode}
        onChange={(e) => { setBillingMode(e.target.value as 'PAY_PER_REQUEST' | 'PROVISIONED'); emit(tableName, hashKey, e.target.value as 'PAY_PER_REQUEST' | 'PROVISIONED') }}
      >
