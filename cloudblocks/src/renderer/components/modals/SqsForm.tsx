...
interface Props { onChange: (p: SqsParams) => void; showErrors?: boolean }
  ...
  return (
    <div>
      ...
      <input
        ...
        onChange={(e) => { setName(e.target.value); emit(e.target.value, fifo, visibilityTimeout) }}
      />
      ...
      <input
        ...
        min={0}
        max={43200}
        onChange={(e) => { setVisibilityTimeout(e.target.value); emit(name, fifo, e.target.value) }}
      />
      ...
      <label style={checkRow}>
        <input
          type="checkbox"
          checked={fifo}
          onChange={(e) => { setFifo(e.target.checked); emit(name, e.target.checked, visibilityTimeout) }}
        />
