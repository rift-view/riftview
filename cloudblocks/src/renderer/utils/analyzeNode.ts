...
export function analyzeNode(node: CloudNode): Advisory[] {
  ...
  if (node.type === 'ecs') {
    ...
    if (typeof desired === 'number' && typeof running === 'number' && running < desired) {
      advisories.push({
        ...
        detail:
          'Fewer tasks are running than desired. Check the ECS service events for placement failures, task exit reasons, or capacity issues.',
        nodeId: node.id
      })
    }
  }
  ...
  if (node.type === 'elasticache') {
    ...
    if (engine === 'redis' && clusterMode === 'standalone') {
      advisories.push({
        ...
        detail:
          'This Redis cluster has no read replica. A node failure will cause an outage until ElastiCache replaces the primary. Add a replica for production workloads.',
        nodeId: node.id
      })
    }
  }
  ...
  if (node.type === 'opensearch' && !node.parentId) {
    advisories.push({
      ...
      detail:
        'This OpenSearch domain has no VPC configuration and may be reachable from the public internet. Place the domain in a VPC and apply fine-grained access control for production workloads.',
      nodeId: node.id
    })
  }
