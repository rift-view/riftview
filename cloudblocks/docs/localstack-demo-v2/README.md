# Cloudblocks LocalStack Demo v2

A comprehensive Terraform fixture that creates a realistic "static website with API" architecture in LocalStack. Exercises every service type Cloudblocks can visualise, intentionally misconfigures several resources to trigger advisory rules, and wires up integration edges between services.
...
1. Open Cloudblocks → **Settings** (gear icon) → **Add Profile**
2. Fill in:
   | Field    | Value                    |
   |----------|--------------------------|
...
| S3 | `demo-static-assets-cloudblocks`, `demo-artifacts-cloudblocks` |
...
| ACM | `demo.cloudblocks.local` certificate |
| CloudFront | `demo-distribution` |
| Route 53 | `cloudblocks.local` hosted zone |
...
| SES | `cloudblocks.local` domain identity, `notifications@cloudblocks.local` email identity |
...
| `demo-static-assets-cloudblocks` (S3) | `s3-public-access` | No `aws_s3_bucket_public_access_block` resource — `publicAccessEnabled=true` |
...
| Node | Advisory | What triggers it |
|------|----------|-----------------|
| `demo-static-assets-cloudblocks` (S3) | `s3-no-versioning` | No `aws_s3_bucket_versioning` resource — `versioningEnabled=false` |
...
| Node | Reason |
|------|--------|
| `demo-artifacts-cloudblocks` (S3) | Public access blocked + versioning enabled |
...
Supporting:
  ...
  ACM: demo.cloudblocks.local
  Route53: cloudblocks.local
  SES: cloudblocks.local, notifications@cloudblocks.local
```
